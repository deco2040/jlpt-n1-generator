...
`;
  ...
  조건:
  - 고급 한자 사용 (潜在, 洞察, 顕著, 拝見, 慎重, 綿密, 繊細, 抽象, 敷衍, 杜撰, 所謂, 批准 등등)
  - 문장 내 **밑줄 표시된 한자어** 포함
  - 4개 선택지: 
    - 정답 1개
    - 발음이 유사한 오답 1개
    - 의미가 유사하거나 반대되는 오답 1개
    - 실제 존재하는 한자이지만 맥락상 맞지 않는 단어 1개
  - 선택지는 모두 실제 존재하는 단어이어야 하며, 학습자가 의미로도 혼동할 수 있게 구성
  - 문장은 자연스럽고 N1 수준의 고급 어휘와 복문 구조 사용
  ...
  출력 형식:
  {
    ...
    "correct": 0,
    "explanation": "정답 한자의 읽기, 의미, 문맥에 적합한 이유와 오답이 틀린 이유를 한국어로 해설"
  }`,
  ...
  조건:
  - 고급 문형 사용 (にもかかわらず, を余儀なくされる, ざるを得ない, に際して, をもって, かたがた, にあって, とあいまって 등등)
  - 문장 중 (　)에 적절한 문형을 넣는 문제
  - 4개 선택지 구성:
    - 정답 1개
    - 의미는 유사하지만 쓰임이 다른 오답 1개
    - 형태는 유사하지만 의미가 다른 오답 1개
    - 실제 존재하지만 문맥상 부자연스러운 오답 1개
  - 모든 선택지는 JLPT N1 또는 N2 수준 문법 표현이어야 함
  - 문장은 자연스럽고 실제 비즈니스/학술/공공문서 스타일도 허용
  ...
  출력 형식:
  {
    ...
    "correct": 0,
    "explanation": "정답 문형의 의미, 용법, 문맥 적합성을 한국어로 설명하며, 오답이 틀린 이유도 포함"
  }`,
  ...
  조건:
  - 고급 어휘 사용 (革新, 懸念, 潜在, 顕在, 抽象, 具体, 概念, 本質, 一貫性, 妥協, 緻密 등등)
  - 문장 내 (　)에 들어갈 적절한 단어를 고르는 문제
  - 4개 선택지 구성:
    - 정답 1개
    - 의미가 유사하지만 문맥에 부적절한 단어 1개
    - 자주 혼동되는 동음이의어 또는 형태 유사 단어 1개
    - 일상적이지만 정확한 의미가 다른 단어 1개
  - 문장은 사회, 철학, 경영, 과학, 교육 등 다양한 분야에서 발췌한 스타일로 작성
  - 문맥에 따른 정답 유추가 명확히 가능해야 함
  ...
  출력 형식:
  {
    ...
    "correct": 0,
    "explanation": "정답 단어의 의미와 문맥 적합성, 오답 단어가 왜 부적절한지에 대한 한국어 해설 포함"
  }`,
