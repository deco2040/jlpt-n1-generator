...
import { generateReadingProblemFromAI } from "../utils/readingAI.js";
...
async function generateUniqueProblem(type, apiKey) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const key =
      type === "kanji" ? parsed.question + parsed.underlined : parsed.question;
...
function getPrompt(problemType) {
  ...
  const prompts = {
    ...
    reading:
      jsonOnlyHeader + `JLPT N1 독해 문제 생성... (주제는 Claude가 선택)`,
  };
